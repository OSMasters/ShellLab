...
int builtin_cmd(char **argv) 
{
	...
	return 1;
    else if(!strcmp(argv[0], "jobs")) { /* List all jobs, running or stopped  */
        listjobs(jobs);
        return 1;
    } else if(!strcmp(argv[0], "bg") || !strcmp(argv[0], "fg")) { /* Handle bg/fg commands  */
        do_bgfg(argv);
        return 1;
    }
...
void do_bgfg(char **argv)
{
    ...
    if(argv[1] == NULL) {
        /* No argument following fg/bg, handle this error */
        printf("%s command requires PID or %%jobid argument\n", argv[0]);
        return;
    }

    if(!isnumeric(argv[1])) {
        /* Illegal argument following fg/bg, handle this error */
    ...
    if(argv[1][0] == '%') {
        ...
        if((job = getjobjid(jobs, jid)) == NULL) {
            /* No job with given JID, handle this error */
        ...
        if((job = getjobpid(jobs, pid)) == NULL) {
            /* No process with given PID, handle this error */
...
void waitfg(pid_t pid)
{
    ...
    while(job->state == FG)
        sleep(1); /* block while process is in foreground */
    return;
}
...
void sigchld_handler(int sig) 
{
    ...
    if((pid = Waitpid(-1, &status, WNOHANG|WUNTRACED)) > 0) {
        /* Child process has exited, stopped or terminated */
        if(WIFEXITED(status)) {
